...
Item {
  ...
  readonly property bool keyboardDeviceAvailable: keyboardDevice !== ""
  readonly property bool keyboardAvailable: keyboardDeviceAvailable && keyboardMax > 0
  ...
  function parseDeviceList(text) {
    ...
    for (let i = 0; i < lines.length; i += 1) {
      ...
      if (deviceClass === "backlight" && nextScreen === "")
        nextScreen = deviceName;
      if (deviceClass === "leds" && deviceName.indexOf("kbd_backlight") >= 0) {
        if (nextKeyboard === "" || deviceName.indexOf("::kbd_backlight") >= 0)
          nextKeyboard = deviceName;
      }
    }
  ...
  Process {
    ...
    Component.onCompleted: exited.connect(function (exitCode) {
      ...
      if (exitCode === 0)
        root.parseDeviceList(detectStdout.text);
      else
        root.settled = true;
    })
  }
  ...
  Process {
    ...
    Component.onCompleted: exited.connect(function (exitCode) {
      root.settled = true;
      if (exitCode === 0)
        root.parseBrightness(screenStdout.text, false);
    })
  }
  ...
  Process {
    ...
    Component.onCompleted: exited.connect(function (exitCode) {
      if (exitCode === 0)
        root.parseBrightness(keyboardStdout.text, true);
    })
  }
